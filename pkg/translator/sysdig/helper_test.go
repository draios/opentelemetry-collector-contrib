...
// Test_addMetadata checks addMetadata updates the map it receives correctly based on the label set and metric
// it receives.
func Test_addMetadata(t *testing.T) {
	// run the empty metadata map case
	t.Run("empty_case", func(t *testing.T) {
		mdMap := map[string]*prompb.MetricMetadata{}
		addMetadata(mdMap, nil, validMetrics1[validIntGauge])
		assert.Exactly(t, mdMap, map[string]*prompb.MetricMetadata{})
	})
	...
	tests := []struct {
		name     string
		orig     map[string]*prompb.MetricMetadata
		testCase []testCase
		want     map[string]*prompb.MetricMetadata
	}{
		{
			"two_metadata",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			addMetadata(tt.orig, tt.testCase[0].labels, tt.testCase[0].metric)
			addMetadata(tt.orig, tt.testCase[1].labels, tt.testCase[1].metric)
			assert.Exactly(t, tt.want, tt.orig)
		})
	}
}
