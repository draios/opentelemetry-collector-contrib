...
func addSample(
	...
	//prompb.MetricMetadata{}.Type.String()
	//metric.Type()
...
func addMetadata(
	mdMap map[string]*prompb.MetricMetadata,
	labels []prompb.Label,
	metric pmetric.Metric,
) string {

	if labels == nil || mdMap == nil {
		return ""
	}

	sig := timeSeriesSignature(metric.Type().String(), &labels)
	_, ok := mdMap[sig]

	if !ok {
		newMd := &prompb.MetricMetadata{
			Type:             resolveMetricType(metric),
...
func resolveMetricType(metric pmetric.Metric) prompb.MetricMetadata_MetricType {
...
func addSingleHistogramDataPoint(
	...
	tstamp := convertTimeStamp(pt.Timestamp())
	...
	if pt.HasSum() {
		...
		addMetadata(mdMap, sumlabels, metric)
	}
	...
	addMetadata(mdMap, countlabels, metric)
	...
	for i := 0; i < pt.ExplicitBounds().Len() && i < pt.BucketCounts().Len(); i++ {
		...
		addMetadata(mdMap, labels, metric)
	...
	addMetadata(mdMap, infLabels, metric)
	...
	if settings.ExportCreatedMetric && startTimestamp != 0 {
		...
		addMetadata(mdMap, createdLabels, metric)
	}
}
...
func addSingleSummaryDataPoint(
	...
	ti := convertTimeStamp(pt.Timestamp())
	...
	for i := 0; i < pt.QuantileValues().Len(); i++ {
		...
		addMetadata(mdMap, qtlabels, metric)
	}
