...
// Config defines configuration for Sysdig Remote Write exporter.
type Config struct {
...
func (cfg *Config) Validate() error {
	if cfg.HTTPClientSettings.Endpoint == "" {
		return fmt.Errorf("remote write endpoint not specified")
	}

	_, err := url.Parse(cfg.HTTPClientSettings.Endpoint)
	if err != nil {
		return fmt.Errorf("error in parsing the remote write endpoint address")
	}

	if cfg.BearerToken == "" {
		return fmt.Errorf("access token not specified")
	}
