...
func batchMetadata(mdMap map[string]*prompb.MetricMetadata, maxBatchByteSize int) ([]*prompb.WriteRequest, error) {
	if len(mdMap) == 0 {
		return nil, errors.New("invalid metadata mdMap: cannot be an empty map")
	}

	var requests []*prompb.WriteRequest
	var mdArray []prompb.MetricMetadata
	currentBatch := 0

	for _, md := range mdMap {
		if currentBatch >= maxBatchByteSize {
			requests = append(requests, &prompb.WriteRequest{Metadata: mdArray})
			currentBatch = 0
			mdArray = nil
		}
