...
func getMetadata(metric pmetric.Metric) *prompb.MetricMetadata {
...
func getMetadataMap(timeseries []*prompb.MetricMetadata) map[string]*prompb.MetricMetadata {
	mdMap := make(map[string]*prompb.MetricMetadata)
	for i, v := range timeseries {
		mdMap[fmt.Sprintf("%s%d", "metadata_name", i)] = v
	}
	return mdMap
}
