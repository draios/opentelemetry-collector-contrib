...
func Test_NewPRWExporter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			prwe, err := newSysdigExporter(cfg, tt.set)
...
func Test_export(t *testing.T) {
	...
	md1 := getMetadata(validMetrics1[validSum])
	handleFunc := func(w http.ResponseWriter, r *http.Request, code int) {
		...
		assert.Equal(t, "234", r.Header.Get("X-Scope-OrgID"))
		assert.Equal(t, "snappy", r.Header.Get("Content-Encoding"))
		assert.Equal(t, "application/x-protobuf", r.Header.Get("Content-Type"))
		assert.Equal(t, "OpenTelemetry Sysdig Prometheus RW sink", r.Header.Get("User-Agent"))
		assert.Equal(t, "PROMETHEUS_NON_COMPLIANT", r.Header.Get("Sysdig-Custom-Metric-Category"))
		...
		if writeReq.GetTimeseries() == nil {
			assert.EqualValues(t, 1, len(writeReq.Metadata))
			assert.Equal(t, *md1, writeReq.GetMetadata()[0])
		} else {
			assert.EqualValues(t, 1, len(writeReq.Timeseries))
			assert.EqualValues(t, 0, len(writeReq.Metadata))
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := runExportPipeline(ts1, md1, serverURL)
			if tt.returnErrorOnCreate {
				assert.Error(t, err)
				return
			}
...
func TestNoMetricsNoError(t *testing.T) {
	...
	serverURL, uErr := url.Parse(server.URL)
	assert.NoError(t, uErr)
	assert.NoError(t, runExportPipeline(nil, nil, serverURL))
}

func runExportPipeline(ts *prompb.TimeSeries, md *prompb.MetricMetadata, endpoint *url.URL) error {
	...
	sysdigExp, err := newSysdigExporter(cfg, set)
	if err != nil {
		return err
	}

	if err = sysdigExp.Start(context.Background(), componenttest.NewNopHost()); err != nil {
		return err
	}

	return sysdigExp.handleExport(context.Background(), testmap, metadataMap)
}
...
func Test_PushMetrics(t *testing.T) {
	...
	checkFunc := func(t *testing.T, r *http.Request, expectedTimeseries int, expectedMetadata int, isStaleMarker bool) {
		body, err := io.ReadAll(r.Body)
		if err != nil {
			t.Fatal(err)
		}
		...
		require.Nil(t, ok)
		if wr.Timeseries == nil {
			assert.EqualValues(t, expectedMetadata, len(wr.Metadata))
		} else {
			assert.EqualValues(t, expectedTimeseries, len(wr.Timeseries))
			if isStaleMarker {
	...
	tests := []struct {
		...
		reqTestFunc        func(t *testing.T, r *http.Request, expectedTimeSeries int, expectedMetadata int, isStaleMarker bool)
				...
				t.Run(tt.name, func(t *testing.T) {
					...
					server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
						if tt.reqTestFunc != nil {
							tt.reqTestFunc(t, r, tt.expectedTimeSeries, tt.expectedMetadata, tt.isStaleMarker)
						}
					...
					set := exportertest.NewNopCreateSettings()
					set.BuildInfo = buildInfo
					prwe, nErr := newSysdigExporter(cfg, set)
...
func TestWALOnExporterRoundTrip(t *testing.T) {
	...
	sysdigExp, perr := newSysdigExporter(cfg, set)
	assert.NoError(t, perr)

	nopHost := componenttest.NewNopHost()
	ctx := context.Background()
	require.NoError(t, sysdigExp.Start(ctx, nopHost))
	t.Cleanup(func() {
		// This should have been shut down during the test
		// If it does not error then something went wrong.
		assert.Error(t, sysdigExp.Shutdown(ctx))
		close(exiting)
	})
	require.NotNil(t, sysdigExp.wal)

	md1 := getMetadata(validMetrics1[validIntSum])
	md2 := getMetadata(validMetrics1[validDoubleGauge])

	mdMap := map[string]*prompb.MetricMetadata{"metadata1": md1, "metadata2": md2}
	...
	errs := sysdigExp.handleExport(ctx, tsMap, mdMap)
	...
	require.NoError(t, sysdigExp.Shutdown(ctx))
	...
	// We must have 2 metadata as were passed into mdMap preceding the timeseries
	// We MUST have 2 time series as were passed into tsMap, after the metadata
	gotMdFromWal := reqs[0]
	assert.Equal(t, 2, len(gotMdFromWal.Metadata))
	gotTsFromWAL := reqs[1]
	assert.Equal(t, 2, len(gotTsFromWAL.Timeseries))
	wantTsReq := &prompb.WriteRequest{
		Timeseries: orderBySampleTimestamp([]prompb.TimeSeries{
			*ts1, *ts2,
		}),
	}
	wantMdReq := &prompb.WriteRequest{
		Metadata: []prompb.MetricMetadata{*md1, *md2},
	}

	// Even after sorting timeseries, we need to sort them
	// also by Label to ensure deterministic ordering.
	orderByLabelValue(gotTsFromWAL)
	gotTsFromWAL.Timeseries = orderBySampleTimestamp(gotTsFromWAL.Timeseries)
	orderByLabelValue(wantTsReq)

	assert.Equal(t, wantMdReq, gotMdFromWal)
	assert.Equal(t, wantTsReq, gotTsFromWAL)
	...
	sysdigExp2, err := newSysdigExporter(cfg, set)
	assert.NoError(t, err)
	require.NoError(t, sysdigExp2.Start(ctx, nopHost))
	t.Cleanup(func() {
		assert.NoError(t, sysdigExp2.Shutdown(ctx))
	})
	require.NotNil(t, sysdigExp2.wal)

	snappyEncodedBytesMdReq, snappyEncodedBytesTsReq := <-uploadedBytesCh, <-uploadedBytesCh
	decodeBufferMd := make([]byte, len(snappyEncodedBytesMdReq))
	decodeBufferTs := make([]byte, len(snappyEncodedBytesTsReq))
	uploadedBytesMd, mderr := snappy.Decode(decodeBufferMd, snappyEncodedBytesMdReq)
	require.NoError(t, mderr)
	uploadedBytesTs, tserr := snappy.Decode(decodeBufferTs, snappyEncodedBytesTsReq)
	require.NoError(t, tserr)
	gotMdFromUpload := new(prompb.WriteRequest)
	gotTsFromUpload := new(prompb.WriteRequest)
	upmderr := proto.Unmarshal(uploadedBytesMd, gotMdFromUpload)
	assert.NoError(t, upmderr)
	uptserr := proto.Unmarshal(uploadedBytesTs, gotTsFromUpload)
	assert.NoError(t, uptserr)

	// Even after sorting timeseries, we need to sort them
	// also by Label to ensure deterministic ordering.
	gotTsFromUpload.Timeseries = orderBySampleTimestamp(gotTsFromUpload.Timeseries)
	orderByLabelValue(gotTsFromUpload)

	fmt.Println(gotMdFromUpload)
	fmt.Println(gotTsFromUpload)

	assert.Equal(t, wantMdReq, gotMdFromUpload)
	assert.Equal(t, wantTsReq, gotTsFromUpload)
}
